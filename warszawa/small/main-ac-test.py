...
# --- Configuration ---
# Set to True to load "model.pth" and run without learning.
# Set to False to train a new model from scratch.
TEST_MODE = True 
...
MAIN_ROAD_MIN_GREEN = 20 
SIDE_ROAD_MIN_GREEN = 5  
...
def run_simulation():
    ...
    if TEST_MODE:
        try:
            ...
            print(">>> LOADED TRAINED MODEL (Testing Mode)")
            model.eval() # Set model to evaluation mode
        except FileNotFoundError:
            print(">>> WARNING: No 'model.pth' found. Testing with random weights.")
            model.eval()
    else:
    ...
    while step < SIM_TIME:
        ...
        if TEST_MODE:
            # Deterministic: Choose the action with highest probability
            action_val = torch.argmax(action_probs).item()
            # We don't need 'dist' or 'action' tensor here
        else:
            # Stochastic: Sample from distribution for exploration
        ...
        if action_val == 1:
            # === SWITCH SEQUENCE ===
            # Apply Switching Penalty (Only relevant for reward tracking in Test Mode)
        ...
        # 4. Learning Step (SKIPPED IN TEST MODE)
        if not TEST_MODE and step < SIM_TIME:
            # Get next state
        ...
        # Logging only (Less frequent printing in test mode usually)
        if TEST_MODE and step % 100 == 0:
             print(f"TEST RUN | Time: {step} | Phase: {current_phase_idx} | "
                   f"Act: {action_val} | Reward: {reward_accumulated:.2f}")

        history.append({'time': step, 'reward': reward_accumulated})

    traci.close()
    pd.DataFrame(data).to_csv("test_log.csv" if TEST_MODE else "training_log.csv")
    if not TEST_MODE:
